...
const Payment = ({ params }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:8000/api/payments/${params.id}`, {
            ...
            <div className="w-full sm:w-1/2">
              ...
              <p
                className={`inline-flex rounded-full bg-opacity-10 py-1 px-3 text-sm font-medium ${
                  data?.status_payment === "Success" ? "text-success bg-success" : data?.status_payment === "Failed" ? "text-danger bg-danger" : "text-warning bg-warning"
                }`}
              >
