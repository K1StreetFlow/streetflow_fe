...
export default function EditCategory({ addressId, onCancel, onEditSuccess }) {
  ...
  return (
    <div>
      <button className="text-white hover:bg-meta-8 bg-warning p-1 px-4 rounded-md me-2" onClick={handleChangeModal}>
        Edit
      </button>

      <input type="checkbox" checked={modal} onChange={handleChangeModal} className="modal-toggle" />

      <div className="modal ">
        <div className="modal-box bg-white ">
          <h3 className="font-bold text-lg text-black">Edit Address</h3>
          <form onSubmit={handleEditSubmit} className="max-w-md mx-auto mt-8 p-4 bg-gray-600 shadow-md rounded-md">
            ...
            <button type="button" onClick={handleChangeModal} className="bg-danger text-white px-4 py-2 rounded-md hover:bg-red-600 focus:outline-none focus:shadow-outline-red active:bg-red-800">
              Cancel
            </button>
          </form>
        </div>
