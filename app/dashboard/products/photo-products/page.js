...
const PhotoProduct = () => {
  ...
  const handleAction = async (productId, action) => {
    if (action === "view") {
      setSelectedProduct(
        photoProducts.find((product) => product.id === productId)
      );
      setShowDetailModal(true);
    } else if (action === "edit") {
      const productToEdit = photoProducts.find(
  ...
  const handleDeleteConfirmation = async () => {
    if (deletingProduct) {
      try {
        ...
        console.error(
          `Error deleting product with ID ${deletingProduct}:`,
          error
        );
            ...
            <div className="bg-white p-8 rounded-md">
              ...
              <div className="flex justify-end mt-4">
                <button
                  ...
                  className="bg-meta-7 hover:bg-meta-1 text-white font-bold py-2 px-4 rounded"
                >
                  Delete
                </button>
                <button
          ...
          >
            ...
            <div className="flex items-center justify-center fixed inset-0">
              <div className="bg-white text-sm mt-12 p-8 rounded-md">
                ...
                <button
                  onClick={() => handleModalToggle("detail")}
                  className=" bg-primary text-white p-2 rounded-sm mt-2"
                >
                  Close
                </button>
              </div>
