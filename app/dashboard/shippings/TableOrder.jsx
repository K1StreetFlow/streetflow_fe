        ...
        setOrders((prevOrders) =>
          prevOrders.map((order) =>
            order.id === orderId
              ? {
                  ...order,
                  order_list: { ...order.order_list, status_order: newStatus },
                }
              : order
          )
        );
          ...
          <tbody>
            {orders?.map((order, key) => (
              <tr key={key}>
                ...
                <td className="border-b border-[#eee] py-5 px-4 pl-9 dark:border-strokedark xl:pl-11">
                  <h5 className="font-medium text-black dark:text-white">
                    {order.address.user_customer.fullname}
                  </h5>
                </td>
                <td className="border-b border-[#eee] py-5 px-4 pl-9 dark:border-strokedark xl:pl-11">
                  <h5 className="font-medium text-black dark:text-white">
                    {order.order_list?.cart.checkout_product
                      .filter((detail) => detail.id_order === order.id)
                      .map((detail) => detail.product.name_product)
                      .join(", ")}
                  </h5>
                </td>
                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                  <p className="text-black dark:text-white">
                    {formatDates(order.createdAt)}
                  </p>
                </td>
                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                  <p className="text-black dark:text-white">
                ...
                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                  <select
                    className="border border-[#eee] rounded-sm py-2 px-3 text-sm font-medium text-white dark:text-white"
