        ...
        <div className="flex w-full flex-wrap ">
          {data.user_customer.address.map((address, key) => (
            <label key={key} className="mb-5">
              <input id="address" className="hidden" type="radio" value={address.id} checked={selectedAddress && selectedAddress.id === address.id} onChange={handleAddressChange} />
              <div className={`p-5 w-100 mx-5 rounded-md shadow-6 hover:bg-gray ${selectedAddress && selectedAddress.id === address.id ? "border-2 border-[#3C50E0]" : ""} `}>
                <div className="">
              ...
              <table className="table w-full py-15">
                ...
                {data.cart_detail?.map((cart, key) => (
                  <tbody key={key}>
                    <tr>
                      <td>
                        <Image src={`http://localhost:8000/api/photo_products/view/${cart.product.photo.photo_product}`} width={200} height={200} className="w-20 rounded" alt="Thumbnail" />
                      </td>
                      <td className="text-base text-black font-bold">{cart.product.name_product}</td>
                      <td className="text-center text-base text-black">Rp {cart.product.price_product.toLocaleString("id-ID")}</td>
