...
export default function Checkout({ data }) {
	...
	async function createOrderList(paymentId, cartId, status_payment) {
		let status_order;
		if (status_payment === "Pending") {
			status_order = "Unpaid";
		} else if (status_payment === "Success") {
			status_order = "Paid";
		} else if (status_payment === "Failed") {
			status_order = "Canceled";
		}

		const orderData = {
			id_payment: paymentId,
			id_cart_details: cartId,
		...
		const response = await axios.post("http://localhost:8000/api/order/create", orderData, config);
			...
			try {
				window.snap.pay(token, {
					onSuccess: async (result) => {
						...
						const response = await axios.post("http://localhost:8000/api/payments", data, config);

						if (response.data) {
							setToken("");
							await createOrderList(response.data.id, cartId, selectedAddress.id, result.status_message);
