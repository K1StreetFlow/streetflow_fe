...
const Address = ({ userData }) => {
  ...
  return (
    <>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      <div className="flex justify-end">
                        <button className="bg-warning text-white px-4 py-2 rounded-md mr-2 hover:bg-blue-700" onClick={() => handleEditAddress(address.id)}>
                          Edit
                        </button>
                        <button className="bg-danger text-white px-4 py-2 rounded-md hover:bg-red-700" onClick={() => handleDeleteAddress(address.id)}>
                          Delete
                        </button>
                      </div>
                    </div>
      ...
      <CreateAddressModal isOpen={isCreateAddressModalOpen} onClose={() => setCreateAddressModalOpen(false)} userId={userData.id} updateAddresses={fetchAddresses} editAddressId={editAddressId} />
      <EditAddressModal isOpen={!isCreateAddressModalOpen && editAddressId !== null} onClose={closeModals} userId={userData.id} updateAddresses={fetchAddresses} editAddressId={editAddressId} />
    </>
  );
};
