  ...
  return (
    <form onSubmit={handleSearchSubmit} className="mb-4 mt-10">
      <div className="flex justify-center mx-auto p-3 w-90 bg-transparent shadow-xl border-b-2  py-2 rounded-xl">
        ...
        />
        <button type="submit" className=" bg-blue-500 hover:bg-blue-700 border-blue-500 hover:border-blue-700 text-sm  text-black py-1 px-2 rounded">
...
const AllProduct = () => {
  ...
  const filteredProducts = products.filter((product) => {
    const categoryMatch =
      selectedCategories.length === 0 || selectedCategories.includes(product.category.name_category_products);
    const colorMatch = selectedColors.length === 0 || selectedColors.includes(product.colour_product);
    const sizeMatch = selectedSizes.length === 0 || selectedSizes.includes(product.size_product);
    const searchMatch =
      searchTerm === '' ||
  ...
  return (
    ...
    <div className="container mx-auto mt-10">
      ...
      <div className="flex justify-center my-4">{renderPaginationButtons()}</div>
    </div>
