...
const ModalDetailTransaksi = ({ id }) => {
  ...
  //   if (product) console.log("PRODUCT", product[0]);
    ...
    <div>
      <div className="p-5 border-b-4 border-y-meta-9">
        <h3
          className={`inline-flex rounded-sm bg-opacity-10 py-1 px-3 text-base font-medium mb-3 ${
            data.data.status_order === "Paid"
              ...
              : data.data.status_order === "Completed"
              ? "text-success bg-success"
              : data.data.status_order === "Packaged"
              ? "text-warning bg-warning"
              : data.data.status_order === "Unpaid"
        ...
        <div className="flex justify-between mb-2">
          <p>Order ID</p>
          <p className="font-semibold">#{data.data.code_order}</p>
        </div>
        <div className="flex justify-between">
          <p>Order Payment</p>
          <p className="font-semibold">{data.data.payment.code_payment}</p>
        </div>
      </div>
      <div className="p-5 border-b-4 border-y-meta-9">
      ...
      <div className="p-5 border-gray-200">
        ...
        <div className="flex justify-between mb-2 border-b border-y-meta-9">
          ...
          <p>{data.data.payment.method_payment}</p>
        </div>
        <div className="flex justify-between mt-4 mb-2">
          <p>Total price</p>
          <p>Rp {data.data.payment.total_payment.toLocaleString("id-ID")}</p>
        </div>
        <div className="flex justify-between">
        ...
        <div className="flex justify-between mt-4 border-t border-y-meta-9">
          ...
          <p className="font-semibold mt-2">
            Rp {data.data.payment.total_payment.toLocaleString("id-ID")}
          </p>
        </div>
      </div>
    </div>
