        ...
        <div className="modal-box">
          ...
          <form onSubmit={handleUpdate}>
            <div className="form-control">
              ...
              <input type="text" className="input w-full input-bordered" placeholder="Customer Name" value={review.user_customer?.fullname || ""} onChange={(e) => setUser(e.target.value)} disabled/>
            </div>
            <div className="form-control">
              <label htmlFor="name_product" className="label font-bold">
                Product Name
              </label>
              <input type="text" className="input w-full input-bordered" placeholder="Product Name" value={review.products?.name_product || ""} onChange={(e) => setProduct(e.target.value)} disabled/>
            </div>
            <div className="form-control">
              <label htmlFor="code_order" className="label font-bold">
                Code Order
              </label>
              <input type="text" className="input w-full input-bordered" placeholder="Code Order" value={review.order_list?.code_order || ""} onChange={(e) => setOrderlist(e.target.value)} disabled/>
            </div>
            <div className="form-control">
              <label htmlFor="message_review" className="label font-bold">
                Message Review
              </label>
              <input type="text" className="input w-full input-bordered" placeholder="Message Review" value={messageReview} onChange={(e) => setMessageReview(e.target.value)} />
            </div>
            <div className="form-control">
              <label htmlFor="number_review" className="label font-bold">
                Number Review
              </label>
              <input type="text" className="input w-full input-bordered" placeholder="Number Review" value={numberReview} onChange={(e) => setNumberReview(e.target.value)} />
            </div>
            <div className="form-control">
              <label htmlFor="photo_review" className="label font-bold ">
                Photo Review
              </label>
              <Image src={getImageUrl(review.photo_review)} width={100} height={100} alt={review.photo_review} className="rounded-md mr-4" />
              <input type="file" id="photoReviewInput" name="photoReview" accept="image/*" className="input w-full input-bordered" placeholder="Photo Review" onChange={(e) => handlePhotoChange(e)} />
              {previewImage && (
            ...
            <div className="modal-action">
              ...
              <button type="submit" className="btn btn-primary hover:btn-primary text-white">
