...
const DropdownUser = () => {
  ...
  const handleLogOut = async () => {
    try {
      ...
      await axios.post("http://localhost:8000/api/admin/auth/logout", null, {
  ...
  useEffect(() => {
    const fetchToken = async () => {
      try {
        ...
        // Panggilan API untuk mendapatkan data pengguna berdasarkan adminId
        const userApiResponse = await axios.get(`http://localhost:8000/api/admin/${decodedToken.adminId}`, {
          withCredentials: true,
        });
    ...
    fetchToken();
    // Reset isReloaded after fetching token
  }, []);
    ...
    <div className="relative">
      <Link ref={trigger} onClick={() => setDropdownOpen(!dropdownOpen)} className="flex items-center gap-4" href="#">
        <span className="hidden text-right lg:block">
          {userDataFromApi ? (
            <>
              <span className="block text-sm font-medium text-black dark:text-white">{userDataFromApi.username}</span>
              <span className="block text-xs">{userDataFromApi.email}</span>
            </>
          ) : (
            <span>Loading...</span>
          )}
      ...
      >
        <ul className="flex flex-col gap-5 border-b border-stroke px-6 py-7.5 dark:border-strokedark">
          <li>
            <Link href="/dashboard/profile" className="flex items-center gap-3.5 text-sm font-medium duration-300 ease-in-out hover:text-primary lg:text-base">
